...
from keyboards.client_inline_keyboards import get_category_list, get_product_list, ad_callback, get_ad_management_keyboard
...
@ads_router.message(Command('add'))
async def add_ad_handler(message: Message, state: FSMContext):
    await new_ad_handler(message, state)

@ads_router.callback_query(ad_callback.filter(action="edit"))
async def edit_ad_handler(callback: CallbackQuery, state: FSMContext):
    ad_id = callback.data['ad_id']
    await state.update_data(ad_id=ad_id)
    await state.set_state(ClientAdsStates.insertText)
    await callback.message.edit_text("Please, send the new text for your ad.")

@ads_router.message(Command('edit_ad'))
async def edit_ad_command_handler(message: Message, state: FSMContext):
    my_ads = db.get_my_ads(message.from_user.id)
    for ad in my_ads:
        keyboard = get_ad_management_keyboard(ad[0])
        await message.answer_photo(
            photo=ad[4],
            caption=f"<b>{ad[1]}</b>\n\n{ad[2]}\n\nPrice: ${ad[3]}",
            parse_mode=ParseMode.HTML,
...
@ads_router.callback_query(ad_callback.filter(action="delete"))
async def delete_ad_handler(callback: CallbackQuery, state: FSMContext):
    ad_id = callback.data['ad_id']
    success = db.delete_ad(ad_id)
    if success:
        await callback.message.edit_text("Your ad has been deleted.")
    else:
        await callback.message.edit_text("Failed to delete the ad. Please try again.")